...
"""
Does same thing as my code but slightly more efficient...
...
while robot.step(TIME_STEP) != -1:
    ...
    if(obj_x>=0 and obj_y >=0): # Object is in the camera image
        ...
        if(obj_r > 0.2): # Still further than 0.2m from the object, drive towards it
            print("  Home in on the object")
            # if obj_b<0, the object is on the right, so left wheel needs to
            # move faster than the right wheel. Difference in speed based on 
            # the angle, so that we rotate slower when the object is in the center
            leftSpeed = (0.5-obj_b) * MAX_SPEED
            rightSpeed = (0.5+obj_b) * MAX_SPEED
            print(obj_b, leftSpeed, rightSpeed)
        else:
            print("  We are there")
            leftSpeed = 0
            rightSpeed = 0
        ...
        #spin the robot by assigning opesed values to the wheels
