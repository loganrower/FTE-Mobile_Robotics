...
while robot.step(TIME_STEP) != -1:
    ...
    if(obj_x>=0 and obj_y >=0): # Object is in the camera image
        ...
        # so now stoping the rotation and will focus on getting this...
        # get the bearings
        obj_r, obj_b = getRangeAndBearing(obj_x, obj_y, depth)
        bearing = obj_b
        
        # stop vehicle if below .2
        if obj_r < .2:
            leftSpeed  = 0 # negative
            rightSpeed = 0  # positive
        elif obj_b > 0: # This means that if it is positive then needs to go left
            print("Left",bearing, obj_r)
            rightSpeed = ((1-obj_b) * MAX_SPEED) #positive
            # added the plus 1 so that speed can go to zero for the negative part
            leftSpeed= ((obj_b) * MAX_SPEED)  # negative
        elif obj_b <= 0: # go right
            print("Right", bearing,obj_r)
            # added the plus 1 so that speed can go to zero for the negative part
            rightSpeed  = (-1*(obj_b) * MAX_SPEED) # negative
            leftSpeed = ((1+obj_b) * MAX_SPEED)  # positive
        ...
        # if none of the above occur then it will just drive toward the block in a straight
        # line...
        else:
            leftSpeed  = 0.5 * MAX_SPEED
            rightSpeed = 0.5 * MAX_SPEED
        
    elif (obj_x < 0 or obj_y < 0): # if there is no contour detected it should rotate on the spot
        leftSpeed = .05*MAX_SPEED
        rightSpeed = 0         
    else:  
