"""tree_row_centered controller."""

"""
This controller deals with centering the 
robot inbetween the trees and also
making sure that it checks the center 
throughout

So in this case what happens is one coordinate that has been taken
on the positive y side (left side) and one negative coordinate on the negative
y side are taken and used to compute the distance across. Then by dividing that by two 
is the center

Then robot will go toward the center and recalibrate..
# THIS WILL ONLY HAPPEN ONCE FOR WHEN STEP =0

# EVERY OTHER TIME THE DISTANCE WILL BE CHECKED TO SEE IF IT MATCHES
## IF IT DOES MATCH WITHIN .05 then all is good. If not then need to essentially 
# Redo the calcualation
"""
import numpy as np
import math
...
while robot.step(TIME_STEP) != -1:
    ...
    slope, intercept =  line_coef_left
