...
class PRMPlanner(PlannerBase):
    ...
    def _create_roadmap(self, npoints, dist_thresh, animate=None):
        ...
        for vertex in self.graph:
            ...
            for othervertex in self.graph:
                ...
                distances.append((vertex.distance(othervertex.coord), othervertex))
    ...
    def plot_graph(self, *args, vertex={}, edge={}, **kwargs):
        """
        Plot PRM path
...
if __name__ == "__main__":
    ...
    prm.plot(
        ax=plt.gca(), text=False, vertex=dict(markersize=4), edge=dict(linewidth=0.5)
    )
    plt.show(block=True)
