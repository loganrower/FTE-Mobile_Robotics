...
from scripts.PlannerBase import PlannerBase
...
class DistanceTransformPlanner(PlannerBase):
    ...
    def plan(self, goal=None, animate=False, animate_nb=False, verbose=False):
        r"""
...
def distancexform(occgrid, goal, metric="cityblock", animate=False, animate_nb=False, summary=False):
    """
    ...
    while True:
        ...
        if animate:
            ...
            if h is None:
                ...
                cmap.set_bad("red")
                cmap.set_over("white")
                h = plt.imshow(display, cmap=cmap, origin='lower')
